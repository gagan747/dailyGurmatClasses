...
function Navbar() {
	...
	const [pathDutiesModalLoader, setPathDutiesModalLoader] = useState(false);
		...
		try {
			...
			const userBookings = data.duties.filter((duty) => {
				return (
					...
					duty.booked?.bookedBy?.username === localStorage.getItem('username')
				);
			});
			setUserBookingsLength(userBookings.length);
			setUserBookedAshtpadi(
				userBookings.length === 1 ? userBookings[0].ashtpadi : null,
			);
			return setPathDuties(data.duties);
		} catch (err) {
			toast.error(messageCleaner(err), { toastId: 'navbarerror' });
		} finally {
			setPathDutiesModalLoader(false);
		}
	};
	...
	const bookAshtpadi = async (ashtpadi) => {
		try {
			...
			const data = await res.json();
			if (res.status === 307) return navigate('/login');
		...
		<>
			{pathDutiesModal && (
				<div className='pathDutiesModal'>
					...
					{pathDutiesModalLoader ? (
						<i
							className='fa fa-cog fa-spin fa-5x fa-fw '
							style={{
								display: 'flex',
